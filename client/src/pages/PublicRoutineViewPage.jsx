...
import {
  ArrowLeft, Loader2, FileText, FileSpreadsheet, Calendar, Clock,
        ...
        <div className="space-y-6">
          ...
          <div className="bg-linear-to-br from-ocean-900 to-ocean-800 rounded-2xl px-4 sm:px-6 py-4 sm:py-5 text-white border border-sky-500/15 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="grid grid-cols-1 sm:flex sm:flex-wrap items-center gap-2 sm:gap-3 w-full md:w-auto">
              <button
                type="button"
                onClick={() => exportRoutineToCsv({
                  assignments: routineDetail?.assignments || [],
                  teachers: routineDetail?.teachers || [],
                  filename: `${meta.department || 'Department'}_${meta.year || '2026'}_${meta.term || 'Routine'}`,
                })}
                disabled={!routineDetail?.assignments?.length}
                ...
                title="Download Tabular CSV for Class Notifications & Spreadsheets"
              >
              ...
              <ExportRoutineDropdown
                batchId={routineDetail?.batch_id || routineDetail?.upload_batch_id || routineDetail?.batch?.id}
                assignments={routineDetail?.assignments || []}
                teachers={routineDetail?.teachers || []}
                filename={`${meta.department || 'Department'}_${meta.year || '2026'}_${meta.term || 'Routine'}`}
                disabled={!routineDetail?.assignments?.length}
